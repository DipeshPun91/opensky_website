...
export default function ContactContent({ config }: { config: SiteConfig }) {
  const { openBookingDialog } = useBookingDialog();

  // Built here (not stored in config) since it depends on all three
  // values together. Uses Google's plain "output=embed" iframe format,
  // which accepts raw lat/lng directly and needs no API key — unlike
  // the "pb=" format the previous hardcoded iframe used, which encodes
  // an opaque place identifier generated by Google's own UI and can't
  // be reconstructed from coordinates alone.
  const mapEmbedSrc = `https://maps.google.com/maps?q=${config.mapLat},${config.mapLng}&z=15&output=embed`;
