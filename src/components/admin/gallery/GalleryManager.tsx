  ...
  const handleAddImage = async (media: MediaItem) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        showError(
          data.error || "Could not add that image.",
          "The image could not be added to the gallery",
        );
        return;
      }
      ...
      showSuccess(
        ...
        "The image has been added to your gallery.",
      );
    } catch {
  ...
  const saveCaption = async (id: string) => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        showError(
          data.error || "Could not save that caption.",
          "The caption could not be updated",
        );
        return;
      }
      ...
      showSuccess(
        "Caption updated!",
        "The caption has been saved successfully.",
      );
    } catch {
      setErrorMessage("Could not reach the server. Please try again.");
      showError(
  ...
  const handleConfirmDelete = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        showError(
          data.error || "Could not remove that image.",
  ...
  const handleMove = async (item: GalleryItem, direction: "up" | "down") => {
    ...
    try {
      ...
      if (!res.ok) {
        ...
        showSuccess(
          "Order updated!",
          `Image ${direction === "up" ? "moved up" : "moved down"} in the gallery.`,
        );
      }
    } catch {
      setErrorMessage("Could not reach the server. Please try again.");
