...
export default async function BlogDetailPage({ params }: BlogDetailPageProps) {
  ...
  // Get other blogs for sidebar (excluding current)
  const otherBlogs = blogPosts.filter((p) => p.slug !== post.slug).slice(0, 4);
          ...
          <div className="lg:col-span-8">
            ...
            <div className="flex flex-col gap-5">
              {post.content.map((paragraph, i) => (
                <p
                  key={i}
                  className="text-sm sm:text-base leading-7 sm:leading-8 text-gray-700"
                >
                  {paragraph}
                </p>
              ))}
